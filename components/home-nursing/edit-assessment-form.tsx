...
import {
  updateHomeNursingAssessmentAction,
...
export function EditAssessmentForm({
  ...
  const actionWithId = updateHomeNursingAssessmentAction.bind(null, assessmentId);
  ...
  return (
    <form action={formAction} className="mt-3 flex flex-col gap-3 rounded border border-slate-200 bg-white p-3">
      ...
      <div className="flex flex-col gap-1">
        <label htmlFor="careNeeds" className={labelClass}>Care needs</label>
        <textarea id="careNeeds" name="careNeeds" rows={2} defaultValue={initial.careNeeds ?? ""} className={inputClass} />
      </div>
