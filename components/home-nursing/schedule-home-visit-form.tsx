...
import {
  scheduleHomeNursingVisitAction,
  type ScheduleHomeNursingVisitState,
} from "@/lib/actions/appointment";

const inputClass = "rounded border border-slate-300 px-3 py-2 text-sm";
const labelClass = "text-sm font-medium text-slate-700";

export function ScheduleHomeVisitForm({ patientId }: { patientId: string }) {
  const actionWithPatientId = scheduleHomeNursingVisitAction.bind(null, patientId);
  const [state, formAction, isPending] = useActionState<
    ScheduleHomeNursingVisitState,
    FormData
